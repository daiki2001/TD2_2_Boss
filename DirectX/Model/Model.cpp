...
ID3D12Device *Model::dev;
UINT Model::descHandleIncrementSize;					//デスクリプタサイズ
...
bool Model::InitializeDiscriptorHeap()
{
...
bool Model::CreateFromObj(string modelName)
{
	HRESULT result = S_FALSE;
...
bool Model::LoadMaterial(const string &directoryPath, const string &filename)
{
	ifstream file;
	file.open(directoryPath + filename);
	if (file.fail()) {
...
void Model::Draw(ID3D12GraphicsCommandList *cmdList, ComPtr<ID3D12Resource> constBuffB0, ComPtr<ID3D12Resource> constBuffB1)
{
	//頂点バッファの設定
